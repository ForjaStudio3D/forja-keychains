(() => {
 ...
 function reveal(){clearTimeout(curtain);curtain=0;
  if(!overlay||overlay.hasAttribute('data-revealed'))return;
  overlay.setAttribute('data-revealed','');
  document.dispatchEvent(new Event('forja:hero-settled'));
 }
 function hideTitle(){clearTimeout(curtain);curtain=0;
  if(!overlay)return;overlay.removeAttribute('data-revealed');
  document.dispatchEvent(new Event('forja:hero-restart'));
 }
 function fadeSeconds(){
  if(!overlay)return .62;
  const declared=parseFloat(getComputedStyle(overlay).transitionDuration);
  return Number.isFinite(declared)&&declared>0?declared:.62;
 }
 // O texto termina de entrar no MESMO instante em que o elenco para: os dois
 // assentam juntos. Por isso o fade comeca uma duracao antes do fim do filme,
 // em vez de esperar o 'ended' e chegar atrasado.
 function scheduleReveal(){
  if(!overlay||overlay.hasAttribute('data-revealed'))return;
  clearTimeout(curtain);
  const total=Number.isFinite(video.duration)&&video.duration>0?video.duration:4;
  const restante=total-video.currentTime-fadeSeconds();
  curtain=setTimeout(reveal,Math.max(0,restante*1000));
 }
 // Rede de seguranca: se o filme nao comecar, o texto entra assim mesmo.
 ...
 video.addEventListener('playing',()=>{clearTimeout(watchdog);playButton.hidden=true;scheduleReveal();});
 // Se a reproducao engasgar e voltar, o alvo e recalculado a partir de onde ela esta.
 video.addEventListener('timeupdate',()=>{
  if(!overlay||overlay.hasAttribute('data-revealed')||video.paused)return;
  const total=Number.isFinite(video.duration)&&video.duration>0?video.duration:4;
  if(video.currentTime>=total-fadeSeconds())reveal();
 });
...
(() => {
 ...
 let released=false;
 document.addEventListener('forja:hero-settled',()=>{released=true;schedule();});
 document.addEventListener('forja:hero-restart',()=>{released=false;index=0;word.textContent=words[0];word.dataset.word=0;stop();});
 if(document.querySelector('.hero-overlay')?.hasAttribute('data-revealed')){released=true;schedule();}
